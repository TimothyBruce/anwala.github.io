...
def getPDFLinks():
    ...
    cl = []                                                         #A list of the content length of the PDF files.
    ...
    for link in links:                                              #For every link on the page,
        ...
        try:                                                        #   front of the link if necessary.
            header = GetURLHeader(link_url)                         #Get the header, which is how the major browsers determine
            content_type = header.get('Content-Type')               #Get the content type from the header, which is how the major browsers determine
    ...
    if len(pdfs) != 0:                                              #Prints the output.
        ...
        for i in range(len(pdfs)):
            print(pdfs[i] + " is a PDF that is "+str(cl[i])+" bytes.")
    else:
